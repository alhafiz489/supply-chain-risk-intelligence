...
<head>
    ...
    <title>
        {{ app()->getLocale() === 'id'
            ? 'Edit Pelabuhan - SupplyGuard'
            : 'Edit Port - SupplyGuard'
        }}
    </title>
...
<div class="main-content">
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h2 class="topbar-title">
                    {{ app()->getLocale() === 'id' ? 'Edit Pelabuhan' : 'Edit Port' }}
                </h2>
                <small class="text-muted">
                    {{ app()->getLocale() === 'id'
                        ? 'Perbarui data pelabuhan yang tersimpan di sistem.'
                        : 'Update the port data stored in the system.'
                    }}
                </small>
            </div>
        </div>
    ...
    <main class="page-wrapper">
        <section>
            <h1 class="page-title">
                {{ app()->getLocale() === 'id' ? 'Form Edit Pelabuhan' : 'Edit Port Form' }}
            </h1>
            ...
            <form method="POST" action="{{ route('admin.ports.update', $port) }}">
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        >
                            ...
                            @foreach ($countries as $country)
                                <option
                                    value="{{ $country->id }}"
                                    @selected(old('country_id', $port->country_id) == $country->id)
                                >
                                    {{ $country->name }}
                                </option>
                            @endforeach
                    ...
                    <div class="col-md-6">
                        ...
                        <select
                            ...
                            required
                        >
                            <option value="Low" @selected(old('congestion_level', $port->congestion_level) === 'Low')>
                                {{ app()->getLocale() === 'id' ? 'Rendah' : 'Low' }}
                            </option>
                            <option value="Medium" @selected(old('congestion_level', $port->congestion_level) === 'Medium')>
                                {{ app()->getLocale() === 'id' ? 'Sedang' : 'Medium' }}
                            </option>
                            <option value="High" @selected(old('congestion_level', $port->congestion_level) === 'High')>
                    ...
                    <div class="col-md-6">
                        ...
                        <input
                            ...
                            max="365"
                            name="delay_days"
                            value="{{ old('delay_days', $port->delay_days) }}"
                ...
                <div class="button-wrapper">
                    ...
                    <button type="submit" class="btn btn-primary action-button">
                        {{ app()->getLocale() === 'id' ? 'Perbarui Pelabuhan' : 'Update Port' }}
                    </button>
                </div>
            </form>
        </section>
